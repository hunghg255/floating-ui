...
const PROVIDE_KEY = '__floating-ui__popper'
...
const createPopper = () => defineComponent({
  ...
  created () {
    if (this.autoMinSize) {
      console.warn('[floating-ui] `autoMinSize` option is deprecated. Use `autoSize="min"` instead.')
    }
    if (this.autoMaxSize) {
      console.warn('[floating-ui] `autoMaxSize` option is deprecated. Use `autoBoundaryMaxSize` instead.')
    }
  },
